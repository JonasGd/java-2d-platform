...
public class IntersectionDetector2D {
    ...
    public static boolean pointOnLine(Vector2f point, Line2D line) {
        ...
        if (dx == 0f) return EMath.compare(point.x, line.getStart().x);
        float m = dy / dx;
    ...
    public static boolean circleAndAABB(Circle circle, AABB box) {
        ...
        if (closestPointToCircle.x < min.x) closestPointToCircle.x = min.x;
        else if (closestPointToCircle.x > max.x) closestPointToCircle.x = max.x;

        if (closestPointToCircle.y < min.y) closestPointToCircle.y = min.y;
        else if (closestPointToCircle.y > max.y) closestPointToCircle.y = max.y;
    ...
    public static boolean circleAndBox2D(Circle circle, Box2D box) {
        ...
        //Create a circle in box's local spac
    ...
    public static boolean AABBAndBox2D(AABB b1, Box2D b2) {
        ...
        EMath.rotate(axesToTest[2], b2.getRigidbody().getRotation(), new Vector2f());
        EMath.rotate(axesToTest[3], b2.getRigidbody().getRotation(), new Vector2f());
        for (int i = 0; i < axesToTest.length; i++) {
            if (!overlapOnAxis(b1, b2, axesToTest[i])) return false;
        }
        return true;
